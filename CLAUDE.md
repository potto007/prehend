...
The WSL2 llama-server on localhost:8080 (gemma-4 GGUFs) serves **our custom model** (e.g. `gemma-4-12b-it-sft-kb-v13-sft`), trained to operate in the **RLM pattern** inside the REPL the harness provides; the harness drives it as its **solver/orchestrator**. (The words "teacher"/"student" apply ONLY to training-trajectory generation in rlm-trainer - never to this model in the harness/eval context.) When a run hitting this server times out (`ConnectTimeout`/`APITimeoutError`) but `curl`/another process to the "same" endpoint works and the server log shows `all slots are idle`:

...
Reuses the same OpenAI-compatible server prehend already drives for completions
(set ``base_url``/``api_key`` to the local llama-server). This is the production
:class:`~prehend.memory.embed.EmbeddingBackend`, replacing FinAcumen's pre-baked
